...
PROTEIN_FA=config['stock_references']['proteome']['second_fasta']
...
rule BlastProteome:
    input: proteome="out/{study_group}/combined.proteome.unique.headers_adjusted.fasta", ref_db=PROTEIN_FA
...
rule CreateSnpEffDatabase:
    input: ref_gtf=STOCK_GENOME_GTF, genome_fa=STOCK_GENOME_FASTA, protein_fa=PROTEIN_FA, snpEff_config=os.path.join(PG2_HOME,'utils/snpEff.config')
    ...
    shell: "ln -s {PROTEIN_FA} {PG2_HOME}/utils/snpEff_dbs/{params.db_name}/protein.fa; ln -s {input.ref_gtf} {output.gtf}; ln -s {input.genome_fa} {output.genome_fa}; cat <( echo '{params.db_name}.genome : Human') {input.snpEff_config} > {input.snpEff_config}.new; mv {input.snpEff_config}.new {input.snpEff_config}; snpEff -Xmx32g build -c {input.snpEff_config} -gtf22 -v {params.db_name}"
...
rule MapMutations_noMS:
    input: annotated_vcf=lambda wildcards:expand("out/{study_group}/novel_analysis/snpEff/{{chr}}.snpEff.vcf",study_group=wildcards.study_group if matched_tumor_normal_data else 'experiment'),proteome="out/{study_group}/combined.proteome.unique.headers_adjusted.fasta",proteome_blast="out/{study_group}/novel_analysis/proteome_blast.outfmt6",ref_db=PROTEIN_FA
...
rule MapMutations:
    input: novel_peps="out/{study_group}/novel_analysis/novel_peptides.txt",annotated_vcf=lambda wildcards:expand("out/{study_group}/novel_analysis/snpEff/{{chr}}.snpEff.vcf",study_group=wildcards.study_group if matched_tumor_normal_data else 'experiment'),proteome="out/{study_group}/combined.proteome.unique.headers_adjusted.fasta",proteome_blast="out/{study_group}/novel_analysis/proteome_blast.outfmt6",ref_db=PROTEIN_FA,novelpep_transcript_map="out/{study_group}/novel_analysis/novelPeptide_transcript_map.txt"
    output: analysis='out/{study_group}/novel_analysis/{mutation_type}/{chr}.{mutation_type}.analysis'
    params: n="1", mem_per_cpu="4", R="'rusage[mem=4]'", J="map_{mutation_type}", o="out/logs/novel_analysis/{study_group}.{mutation_type}.out", eo="out/logs/novel_analysis/{study_group}.{mutation_type}.err", \
